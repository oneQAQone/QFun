    ...
    override fun loadHook() {
        ...
        PopupMenuDialog::class.java
            .findMethod {
                ...
                val toRemove = menuItemList.filter {
                    it.id == R.string.app_name || 
                    it.id == R.string.plugin_name || 
                    it.id == R.string.storage_clean
                }
                menuItemList.removeAll(toRemove)

                // 按顺序创建菜单项
                val qfunItem = MenuItem(
                    R.string.app_name,
                    "QFun",
                    "QFun",
                    R.drawable.ic_launcher
                )
                val pluginItem = MenuItem(
                    R.string.plugin_name,
                    "JavaPlugin",
                    "JavaPlugin",
                    R.drawable.ic_float_ball
                )
                val cleanItem = MenuItem(
                ...
                menuItemList.add(0, qfunItem)
                menuItemList.add(1, pluginItem)
                menuItemList.add(2, cleanItem)
